...
| 層 | 内容 | 実写への通用度 | 使い方 |
| --- | --- | --- | --- |
| **A. データセットに依存しないコードの事実** | 前乗算なし warp による毛先RGB誤差（30〜40倍）、FG無しtrimapでの未処理例外、`crop_roi` の INTER_AREA 縮小、oracle行＝matting単体の上限になる性質 | **そのまま有効。実写でも同じ**（合成データではなくコードパスの性質を測っている） | そのまま修正対象にしてよい |
