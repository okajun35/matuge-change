...
- パイプラインの目標は「商品＋商品に隠された自まつ毛」なので、GT（商品のみ）より広い。
  ただし自まつ毛を除外しても Dice 0.522→0.556 なので、これは主因ではない
...
| 商品のRGBを保てたか | RGB MAE / RMSE（両者の alpha >= 0.9 の画素のみ）＋ `opaque_coverage` |
| 再合成で崩れないか | 商品周辺帯の MAE / RMSE（**GT商品を同じbaseに合成した理想像**との差）、`reconstruction_error` |
| 移動・回転でピクセルが変質しないか | exact color preservation / mutation rate / RGB MAE（`evaluation/mutation.py`） |
| 落ちずに完走したか | `success_rate`（失敗ランはNaNで平均から外れるので、これが無いと平均が嘘になる） |

指標の読み方で外せない2点:

- **`rgb_mae` は必ず `opaque_coverage` とセットで見る。** 予測が不透明画素をほとんど落としても、
  残った画素さえ合っていれば `rgb_mae` は小さくなる。coverage はその「根拠にした割合」
- **`grad` は画像サイズに比例する**（合計を1000で割っただけの値）。解像度をまたいで比較するときは
  画素あたりに正規化した `grad_mean` を使う
...
ケースは **ブロック単位**で生成される。1ブロック = 1つの（背景 × 商品）ペアに対する
baseline と全条件（現在 26 ケース）。同じ目に対する baseline があるので、レポートは

```text
条件のDice − 同じ目のbaselineのDice
```

という **paired delta** を出せる。条件と背景を独立に回すと、条件差に「その目の難しさ」が
混ざる（初版はそうなっており、背景効果を補正すると条件平均が最大0.02動いた）。

`--cases` がブロックサイズの倍数でないと最後のブロックが欠け、その分は paired 比較に使えない。
生成時に「何ブロック完成したか」を表示する。

### manifest

生成時に `manifest.json`（ケースIDの一覧）を書き、`load_dataset()` はそこに載ったケースだけ読む。
これが無いと、小さいデータセットを作り直したときに前回の大きい生成物が残って一緒に評価される。
