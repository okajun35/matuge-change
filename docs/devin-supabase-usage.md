...
Devin は単なるコーディングエージェントとしてではなく、**実装・技術調査・設計の壁打ち相手**として利用しました。

主な活用:

- 画像処理パイプライン、API、UI、テストなどの実装
- MediaPipe / OpenCV / PyMatting など、利用技術の実装方法や API の調査・解説
- 静止画の ROI、位置合わせ、Trimap、Alpha Matting などの方式検討
- 動画処理で、毎フレーム独立に Alpha を推定した場合の**ちらつき（Temporal Consistency）**について壁打ち
- 「1枚のまつ毛レイヤーを変形して追従させる」案と
  「元動画の目元領域を保持する」案の比較・検討
- 実装後のテスト、レビュー指摘への修正、ドキュメント更新

特に動画モードでは、単純に静止画処理を全フレームへ適用すると
Alpha がフレーム間で揺れてちらつく問題について相談し、
最終的に **商品側を変形せず、AI加工画像側を各フレームへ位置合わせする方式**
へ整理する際の壁打ちが役立ちました。
