...
- 簡易モードの表示は装着元／AI加工済み／合成結果を主要3項目、切り抜き／Alphaを折りたたみの
  ...
  処理完了後はこの比較モードを最初に表示し、「結果を保存」で左側の合成結果PNGを保存する
- **簡易モードへ現行のカタログ登録を追加しない**。現行カタログは再利用するまつ毛素材
  （Product RGBA / Alpha）の保管場所で、加工の修正・再開に必要な作業一式の保存先ではない。
  後者は「作業履歴」として、装着元、AI加工済み、合成後、切り抜き、Alpha、ROI・ブラシ・
  位置合わせ等のセッション情報をまとめて扱う。まず既存セッションの一覧・比較・詳細再開を
  履歴へ発展させ、比較画面に入力枠や「カタログに登録」ボタンを増やさない。
